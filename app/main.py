...
HERETIC_ARA_SOURCE_DIR = Path(
    os.getenv("HERETIC_ARA_SOURCE_DIR", DATA_DIR / "heretic_upstream_ara")
).resolve()
HERETIC_ARA_VERSION_FILE = DATA_DIR / "heretic_version_ara.json"
HERETIC_ARA_SLOTS_DIR = DATA_DIR / "heretic_slots_ara"
...
class JobRequest(BaseModel):
    ...
    heretic_channel: Literal["master", "ara"] = "master"
    use_ara: bool = True
...
def render_config(request: JobRequest, output_directory: Path, job_id: str) -> str:
    ...
    if request.heretic_channel == "ara":
        ...
        # automation keys exist there via the managed patch in
        # patches/heretic-ara.
        del values["offload_outputs_to_cpu"]
        values.update(
            {
                "use_ara": request.use_ara,
                "use_ara_lora": request.use_ara_lora,
                "ara_lora_rank": request.ara_lora_rank,
...
class JobManager:
    ...
    def _run(self, job_id: str) -> None:
        ...
        with self.lock:
            ...
            self._persist(job)
        try:
            channel_manager = heretic_version_managers.get(
                job.heretic_channel or "master", heretic_version_manager
            )
            runtime = channel_manager.runtime_info(job.heretic_slot)
...
heretic_version_managers = {
    ...
    "ara": HereticVersionManager(
        HERETIC_ARA_SOURCE_DIR,
        HERETIC_ARA_VERSION_FILE,
        HERETIC_ARA_PATCH_DIR,
        slots_dir=HERETIC_ARA_SLOTS_DIR,
        upstream_url=HERETIC_UPSTREAM_URL,
        initial_ref=HERETIC_ARA_INITIAL_REF,
        branch="ara",
    ),
}
heretic_version_manager = heretic_version_managers["master"]
...
def resolve_version_manager(channel: str) -> HereticVersionManager:
    manager = heretic_version_managers.get(channel)
    if manager is None:
        raise HTTPException(status_code=404, detail=f"未知的 Heretic channel：{channel}")
    return manager


@app.get("/api/heretic/version")
def get_heretic_version(check_remote: bool = False, channel: str = "master"):
    manager = resolve_version_manager(channel)
    try:
        return manager.status(check_remote=check_remote)
    except RuntimeError as exc:
...
def update_heretic_version(request: HereticVersionActionRequest):
    ...
    manager = resolve_version_manager(request.channel)

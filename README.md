...
Ollama's adapter converter only accepts a few architectures (Llama, Mistral, Gemma families); other Safetensors adapters fail on import with `unsupported architecture`. For these, the **Merge into full model** form applies the LoRA to a local base model with PEFT on CPU and writes a new complete model into `./outputs`, which can then be imported through the regular Models & Ollama flow, including GGUF conversion. The UI automatically matches each adapter's `base_model_name_or_path` against local outputs, suggests the base model, and flags adapters that require merging. The base field also accepts a typed path under `/models` for local source models, or a Hugging Face model ID (`org/repo`): the worker then downloads the base into the HF cache (resumable, using the stored token) before merging — when no matching local output exists, the adapter's recorded HF base is pre-filled. Ollama models (GGUF) cannot serve as a merge base; entering an Ollama-style tag shows an explanatory error. Merging temporarily needs roughly the base model's size in free system RAM and disk space.
